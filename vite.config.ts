...
function googlePlacesDevPlugin(apiKey: string | undefined): Plugin {
  return {
    ...
    configureServer(server) {
      server.middlewares.use('/google-places/hours', async (request, response) => {
        ...
        try {
          ...
          sendJson(response, { message: 'Google Places request failed.' }, 502);
        }
      });
    },
  };
...
export default defineConfig(({ mode }) => {
  ...
  return {
    plugins: [react(), tailwindcss(), googlePlacesDevPlugin(env.GOOGLE_MAPS_API_KEY)],
    server: {
      proxy: {
        '/kakao-routing': {
